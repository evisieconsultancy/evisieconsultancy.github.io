...
<code class="html" x-include="/admin-console/components/views/Training/get.html"></code>
The source of the above example is located at: /admin-console/components/views/Training/get.html 

#### Basic concept
```html
<draftsman-query alias="Trainingget" x-include="/prepared-statements/views/Training/get.txt">
</draftsman-query>
```
Binds a prepared statement (/prepared-statements/views/Training/get.txt) to an Alpine data object (Trainingget).
...
*Limitation: a draftsman-query tag may only include 1 toplevel query (in the example 'Training'). However, 
...
the [Alpine Store method](https://alpinejs.dev/magics/store) '$store.{alias}.{path}.{name}' e.g. '$store.Trainingget.get.key'. The alias attribute is mandatory to prevent collisions
in the data store.
...
the alias.
```html
<div style="display:none;" id="Trainingget" x-include="/form-definitions/Training/get.json"></div>
```
...
from the view updater indicating that view 'Training' has been updated. It does not filter on identity, so this may be 
...
tag *:identifier=$store.training.get.key*. The ':' tells AlpineJS that the value should be set using a JavaScript statement, 
the statement '$store.training.get.key' refers to previously retrieved data using a query tag.
```html
<draftsman-notification message="updated" type="Training"
    @notification=" 
...
<code class="html" x-include="/admin-console/components/commands/SendMailGeneral.html"></code>

#### The basic concept
```html
<draftsman-mutation command="SendMailGeneral" x-include="/prepared-statements/commands/SendMailGeneral.txt">
</draftsman-mutation>
```
Registers a prepared statement (/prepared-statements/commands/SendMailGeneral.txt) as a command (SendMailGeneral) in the Draftsman framework. 
The body of the tag is imported by vimesh-ui.
...
the command.
```html
<div style="display:none;" id="SendMailGeneral" x-include="/form-definitions/SendMailGeneral.json"></div>
```
...
```javascript
$store.mutation.send('SendMailGeneral',{all-form-data});
```
...
```html
<draftsman-trace command="SendMailGeneral" 
                 status="success" 
                 @trace="
                    Draftsman.empty_track_and_trace_log();
                    $dispatch('setdata', { command: 'SendMailGeneral', data: {} });">
</draftsman-trace>
```
